...
extension IdentityProvider: IdentityProviderProtocol {
	...
	func fetchData(completion: @escaping (IdentityServerModel) -> (), failure: @escaping (NetworkError) -> ()) {
		networkService.requestGeneric(dto: IdentityProviderRequestDTO.requestDataList(param: IdentityRequest(anioNacimiento: 1996,
																											 numeroSoporte: "323135468")),
	...
	func fetchDataAddress(completion: @escaping (AddressServerModel) -> (), failure: @escaping (NetworkError) -> ()) {
		networkService.requestGeneric(dto: AddressProviderRequestDTO.requestDataAddress(param: AddressRequest(español: "s")),
...
struct IdentityProviderRequestDTO {
	static func requestDataList(param: ProviderParamsDTO?) -> RequestDTO {
		RequestDTO(param: param?.encodeData(),
				   method: .post,
				   endpoint: Constants.URL.getIdentity)
	}
}
...
struct AddressProviderRequestDTO {
	static func requestDataAddress(param: ProviderParamsDTO?) -> RequestDTO {
		RequestDTO(param: param?.encodeData(),
				   method: .post,
				   endpoint: Constants.URL.getAddress)
	}
}
